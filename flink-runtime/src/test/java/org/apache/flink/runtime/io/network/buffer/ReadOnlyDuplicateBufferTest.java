 ...
 */
public class ReadOnlyDuplicateBufferTest extends BufferTest {
	private final Random random = new Random();

	@Override
	protected ByteBuf newBuffer(int length, int maxCapacity) {
		NetworkBuffer wrapped = (NetworkBuffer) super.newBuffer(length, maxCapacity);
		ReadOnlyDuplicateNetworkBuffer readOnlyDuplicate = wrapped.readOnlyDuplicate();

		assertEquals(wrapped.writerIndex(), readOnlyDuplicate.writerIndex());
		assertEquals(wrapped.readerIndex(), readOnlyDuplicate.readerIndex());

		return readOnlyDuplicate;
	}
