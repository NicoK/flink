...
public final class BlobClient implements Closeable {
	...
	public InputStream get(JobID jobId, BlobKey blobKey) throws IOException {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("GET BLOB for job {} with key {} from {}.", jobId, blobKey,
	 ...
	 */
	private static void sendGetHeader(OutputStream outputStream, JobID jobId, BlobKey blobKey) throws IOException {
		checkNotNull(jobId);
	 ...
	 * 		thrown if an I/O error occurs while uploading the data to the BLOB server
	 */
	public BlobKey put(JobID jobId, byte[] value, int offset, int len) throws IOException {
	 ...
	 * 		thrown if an I/O error occurs while writing the header data to the output stream
	 */
	private static void sendPutHeader(OutputStream outputStream, JobID jobId) throws IOException {
		checkNotNull(jobId);
